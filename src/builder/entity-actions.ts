...
export const entityActions = <T, A>(featureKey: string) => {
  ...
  const setMany = createAction(
    '[' + featureKey + ' / Store] Set Many ' + featureKey,
    props<{ entities: T[] }>()
  );
  const setOne = createAction(
  ...
  const removeMany = createAction(
    '[' + featureKey + ' / Store] Remove Many ' + featureKey,
    props<{ ids: number[] }>()
  );
  const removePredicate = createAction(
