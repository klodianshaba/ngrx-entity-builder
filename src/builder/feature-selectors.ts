...
export const featureSelectors = <T, A> (featureSelector: MemoizedSelector<object, object>, featureKey: string) => {
    const selectState = createSelector<object, object, T & A>(
        featureSelector,
        (state) => (featureKey) ? state[featureKey] : state
    );

    const selectByProperty = (property: string) => createSelector(
        selectState,
