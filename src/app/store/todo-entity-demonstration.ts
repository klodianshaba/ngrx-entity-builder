...
import { TodoModel } from '../models/book.model';
...
import {
  ...
  todoPartial,
  additionalPartial,
} from '../models/todo-entity-demonstration.model';

export const todoFeatureKey = 'todos';

export const todoEntity = createEntity<State, TodoModel, AdditionalModel>(
  selectState,
  todoFeatureKey
);
...
export class Component {
  ...
  actions() {
    ...
    this.store.dispatch(
      todoEntity.actions.updateOne({ id, changes: todoPartial })
    ); // update one entity
    this.store.dispatch(
    ...
    this.store.dispatch(
      todoEntity.actions.removePredicate(todo => todo.year != 2024)
  ...
  selectors() {
    ...
    this.store.selectSignal(
      todoEntity.selectors.selectPredicate(todo => todo.year == 2024)
    ); // select by predicate
  }
}
